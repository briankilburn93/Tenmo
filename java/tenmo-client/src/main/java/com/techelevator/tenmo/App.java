...
public class App {
	...
	private void viewCurrentBalance(AuthenticatedUser id) {
		...
		ResponseEntity<Accounts> responseEntity = restTemplate
				.getForEntity(API_BASE_URL + "/accounts/searchUserId?userId=" + id.getUser().getId(), Accounts.class);
		account = responseEntity.getBody();
		System.out.println("Your current account balance is $" + account.getBalance());
	}

	/*
	 * private Accounts getUserAccount(int currentUserId) { Accounts usersAccount =
	 * new Accounts(); RestTemplate apiCall = new RestTemplate();
	 * ResponseEntity<Accounts> responseEntity = apiCall.getForEntity(API_BASE_URL +
	 * "/accounts/searchUserId?userId=" + currentUserId, Accounts.class);
	...
	/*
	 * private void getAllAccounts() { RestTemplate restTemplate = new
	 * RestTemplate(); ResponseEntity<Accounts[]> responseEntity =
	 * restTemplate.getForEntity(API_BASE_URL + "/accounts", Accounts[].class);
	 * List<Accounts> allAccounts = Arrays.asList(responseEntity.getBody());
	 * 
	 * listAccounts(allAccounts); } private void listAccounts(List<Accounts>
	 * accounts) { if (accounts.size() > 0){s for(Accounts acc : accounts) {
...
//Writing a note to update this file
	...
	private void sendBucks(AuthenticatedUser id) {
		getAllUsers();
		int authUser = id.getUser().getId();
